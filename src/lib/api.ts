...
export const api = {
  ...
  checkDeps:     ()           => req("/system/deps"),
  metrics:       ()           => req("/metrics"),
  analytics:     ()           => req("/analytics/summary"),
  appSettings:   (id:string)  => req(`/apps/${id}/settings`),
  saveSettings:  (id:string, s:object) => req(`/apps/${id}/settings`, {method:"PUT", body:JSON.stringify(s)}),
  restart:       (id:string)  => req(`/apps/${id}/restart`, {method:"POST"}),
  healthScore:   (id:string)  => req(`/apps/${id}/health-score`),
  forkApp:       (id:string, name:string) => req(`/apps/${id}/fork`, {method:"POST", body:JSON.stringify({name})}),
  appReadme:     (id:string)  => req(`/apps/${id}/readme`),
  templates:     ()           => req("/templates"),
  installTemplate:(id:string) => req(`/templates/${id}/install`, {method:"POST"}),
  nlSearch:      (q:string)   => req("/apps/search-and-install", {method:"POST", body:JSON.stringify({query:q})}),
  aiSetup:       (id:string)  => req(`/apps/${id}/ai-setup`, {method:"POST"}),
  exportApp:     (id:string)  => req(`/apps/${id}/export`),
}
